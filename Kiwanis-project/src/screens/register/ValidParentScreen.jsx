...
const ValidParentScreen = ({ route }) => {
  const { childId } = route.params;

  const handleApproval = async (approved) => {
    // Mettre à jour le statut d'approbation dans Firestore
    try {
      await firestore().collection('parentApprovals').doc(childId).update({
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.text}>Votre enfant souhaite participer au concours photo et a besoin de votre approbation.</Text>
...
export default ValidParentScreen;
